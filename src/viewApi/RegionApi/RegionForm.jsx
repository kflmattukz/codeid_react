...
function RegionForm({ edit, handleEdit }) {
    ...
    <div className='mt-5'>
      <form  onSubmit={ (e) => submitHandler(e) } encType='multipart/form-data' className='flex flex-col w-1/2 gap-3 text-gray-700'>
        { edit.regionId ? 
          ...
          <input className='px-2 py-1 rounded bg-white ring-blue-400' type="text" disabled={true} name="regionId" value={ regionId }/>
          ...
          <input
            ...
            onChange={(e) => handleChange(e)} type="text" name="regionName" id="regionName" />
          <label htmlFor="regionFile" className='font-semibold'>Region File</label>
          <input
            // value={ regionFile }
            onChange={(e) => handleChange(e) } type="file" name="regionFile" id="regionFile" />
          <label htmlFor="regionPhoto" className='font-semibold'>Region Photo</label>
          <input
            // value={ regionPhoto }
            onChange={(e) => handleChange(e) }  type="file" name="regionPhoto" id="regionPhoto" />
          <button type='submit' className='text-white bg-blue-500 hover:bg-blue-600 px-3 py-2 font-semibold rounded-md hover:shadow-md duration-300'>Update Region</button>
          </>
          :
          <>
            <label htmlFor="regionName" className='font-semibold'>Region Name</label>
      ...
      <p className='mt-5 text-gray-700 font-semibold text-lg'>{ notif ? edit.regionId ? 'Update Region Success': 'Add Region Success' : ''}</p>
    </div>
  )
}
